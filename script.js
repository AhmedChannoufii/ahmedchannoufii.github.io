...
const translations = {
  en: {
    ...
    "proj.pfe.desc": "End-to-end Machine Learning decision-support platform built for Tunisie Telecom following the <strong>CRISP-DM</strong> methodology. The target file combined basic customer profile information along with the target variable (customer status for the following month), supplemented by <strong>5 behavioral data files covering 12 months</strong> (outgoing/incoming calls, internet consumption, recharges, USSD). Cleaned and consolidated these files around the customer ID to construct <strong>46 behavioral features</strong> (trend deltas, active months, recency). Balanced dataset with <strong>SMOTE</strong> and deployed in an interactive <strong>Flask Web Dashboard</strong> with automated multi-file ingestion, dynamic KPI analytics, and CSV reporting.",
  ...
  fr: {
    ...
    "proj.pfe.desc": "Plateforme décisionnelle de Machine Learning de bout en bout développée pour Tunisie Telecom suivant la méthodologie <strong>CRISP-DM</strong>. Le fichier cible regroupait les informations de base des clients ainsi que la variable cible (statut du client le mois suivant), complété par <strong>5 fichiers de données comportementales couvrant 12 mois</strong> (appels sortants/entrants, consommation internet, recharges, USSD). Nettoyage et consolidation de ces fichiers autour de l'identifiant client pour construire <strong>46 caractéristiques comportementales</strong> (tendances, mois actifs, récence). Jeu de données équilibré avec <strong>SMOTE</strong> et déployé dans un <strong>tableau de bord Flask interactif</strong> avec ingestion automatisée multi-fichiers, analytique KPI dynamique et export CSV.",
